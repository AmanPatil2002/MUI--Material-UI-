...
exports.DEFAULT_OVERFLOW_CHIP_WIDTHS = exports.DEFAULT_GAP = void 0;
exports.calculateVisibleCount = calculateVisibleCount;
exports.getOverflowChipWidth = getOverflowChipWidth;
/**
 ...
 */
const DEFAULT_OVERFLOW_CHIP_WIDTHS = exports.DEFAULT_OVERFLOW_CHIP_WIDTHS = [32, 38, 44];

/** Fallback gap between sibling chips, mirrors the default `gap: 4` in `MultiSelectChips`. */
const DEFAULT_GAP = exports.DEFAULT_GAP = 4;
 ...
 */
function getOverflowChipWidth(hiddenCount, widths = DEFAULT_OVERFLOW_CHIP_WIDTHS) {
  if (hiddenCount <= 0) {
    return 0;
  }
  if (widths.length === 0) {
 ...
 */
function calculateVisibleCount(arrayLength, containerWidth, chipWidths, overflowChipWidths = DEFAULT_OVERFLOW_CHIP_WIDTHS, gap = DEFAULT_GAP) {
  let usedWidth = 0;
  let count = 0;
  for (let i = 0; i < arrayLength; i += 1) {
    ...
    const spaceNeeded = usedWidth + chipWidth + (count > 0 ? gap : 0) + (hiddenIfStopHere > 0 ? overflowChipWidth + gap : 0);
