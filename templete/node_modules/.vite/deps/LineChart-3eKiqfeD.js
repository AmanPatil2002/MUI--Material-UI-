...
import { s as generateUtilityClasses, t as styled } from "./styled-DR8vx7FB.js";
...
import { Ft as useDrawingArea, H as useItemHighlightState, It as useStore, L as useChartGradientIdBuilder, Lt as useChartsContext, M as useAnimateArea, O as constant_default, P as ANIMATION_TIMING_FUNCTION, Pt as useChartId, Rt as getValueToPositionMapper, St as useXAxes, V as useItemHighlightStateGetter, Z as useLineSeriesContext, ct as useChartsLayerContainerRef, jn as selectorChartSeriesConfig, k as useAnimateLine, on as selectorChartZoomMap, p as useFocusedItem, sn as selectorChartZoomOptionsLookup, un as isOrdinalScale, wt as useYAxes, x as withPath } from "./hooks-CYJjzrvT.js";
import { n as reactMajor_default } from "./useStore-Df0ffzi5.js";
import { $ as Symbol, A as useSkipAnimation, Ct as useChartCartesianAxis, F as ChartsTooltip, Ft as selectorChartExperimentalFeaturesState, H as getSymbol, Lt as useChartVisibilityManager, Mt as getChartPoint, Nt as getAxisIndex, Pt as useChartZAxis, St as useChartBrush, T as ChartsOverlay, _ as getColor, _t as useChartHighlight, bt as useChartTooltip, et as symbolsFill, g as getCurveFactory, i as ChartsSurface, j as ChartsLegend, jt as useChartInteraction, l as ChartsDataProvider, nt as array_default, r as useChartsContainerProps, t as ChartsWrapper, tt as linear_default, yt as useChartKeyboardNavigation } from "./ChartsWrapper-XBLrLLWB.js";
...
import { _ as selectorChartSamplingState, a as ChartsGrid, g as selectorChartSamplingPyramids, h as getFullRangeZoom, m as ChartsClipPath, n as ChartsAxisHighlight, s as ChartsAxis, t as useInternalIsZoomInteracting, v as selectorChartsHighlightXAxisIndex } from "./useInternalIsZoomInteracting-bLeFoCOE.js";
import { n as useInteractionItemProps } from "./useInteractionItemProps-1MyKvz6L.js";
...
//#endregion
//#region node_modules/@mui/x-charts/LineChart/useLineItemClickHandler.mjs
/**
...
function useLineItemClickHandler(onItemClick) {
	...
	return import_react.useMemo(() => {
		...
		return (event, seriesId) => {
			...
			const xAxis = xAxisId === void 0 ? void 0 : xAxes[xAxisId];
			if (element === null || xAxis === void 0) return;
			const dataIndex = getAxisIndex(xAxis, getChartPoint(element, event).x);
...
function AreaPlot(props) {
	...
	return /*#__PURE__*/ (0, import_jsx_runtime.jsx)(AreaPlotRoot, _extends({ className: clsx(classes.areaPlot, className) }, other, { children: completedData.map(({ d, seriesId, color, area, gradientId }) => !!area && /*#__PURE__*/ (0, import_jsx_runtime.jsx)(AreaElement, {
		seriesId,
		d,
		color,
		gradientId,
...
function useLinePlotData(xAxes, yAxes, samplingOverride) {
	const seriesData = useLineSeriesContext();
	const defaultXAxisId = useXAxes().xAxisIds[0];
	const defaultYAxisId = useYAxes().yAxisIds[0];
	const getGradientId = useChartGradientIdBuilder();
	...
	const wasSamplingEnabled = import_react.useRef(samplingEnabled);
	...
	return import_react.useMemo(() => {
		...
		for (const stackingGroup of stackingGroups) {
			...
			for (const seriesId of groupIds) {
				...
				const zoom = zoomMap?.get(xAxisId) ?? (built ? getFullRangeZoom(xAxisId) : void 0);
				let sampledBuckets = null;
				if (samplingEnabled && built && zoom && !shouldExpand && xData) sampledBuckets = sampler?.sample?.({
					built,
					zoom,
				...
				let formattedData;
				if (sampledBuckets) formattedData = sampledBuckets.flatMap((bucket) => Array.from(bucket.indices, (index) => ({
					x: xData[index],
					y: visibleStackedData[index],
					nullData: data[index] == null
				})));
				else formattedData = xData?.flatMap((x, index) => {
					const nullData = data[index] == null;
					if (shouldExpand) {
						const rep = [{
							x,
...
function LinePlot(props) {
	...
	return /*#__PURE__*/ (0, import_jsx_runtime.jsx)(LinePlotRoot, _extends({ className: clsx(classes.linePlot, className) }, other, { children: completedData.map(({ d, seriesId, color, gradientId, hidden, isSampled }) => {
	...
	return import_react.useMemo(() => {
		...
		for (const stackingGroup of stackingGroups) {
			...
			for (const seriesId of groupIds) {
				...
				if (!showMark || showMark === "start" || showMark === "end") continue;
				if (!(xAxisId in xAxes) || !(yAxisId in yAxes)) continue;
...
function MarkPlot(props) {
	...
	const highlightedItems = import_react.useMemo(() => {
		const rep = {};
		for (const { dataIndex, axisId } of xAxisHighlightIndexes) if (rep[axisId] === void 0) rep[axisId] = /* @__PURE__ */ new Set([dataIndex]);
		else rep[axisId].add(dataIndex);
		return rep;
	}, [xAxisHighlightIndexes]);
	const completedData = useMarkPlotData(xAxis, yAxis);
	const classes = useUtilityClasses();
	return /*#__PURE__*/ (0, import_jsx_runtime.jsx)(MarkPlotRoot, _extends({ className: clsx(classes.markPlot, className) }, other, { children: completedData.map(({ seriesId, clipId, shape, xAxisId, marks, hidden }) => {
...
function LineHighlightPlot(props) {
	...
	const hasStartEndMark = stackingGroups.some(({ ids }) => ids.some((seriesId) => series[seriesId].showMark === "start" || series[seriesId].showMark === "end"));
	if (highlightedIndexes.length === 0 && !hasStartEndMark) return null;
	const getHighlightedIndex = /* @__PURE__ */ new Map();
	...
	return /*#__PURE__*/ (0, import_jsx_runtime.jsx)("g", _extends({}, other, { children: stackingGroups.flatMap(({ ids: groupIds }) => groupIds.flatMap((seriesId) => {
		const { xAxisId = defaultXAxisId, yAxisId = defaultYAxisId, visibleStackedData, data, disableHighlight, showMark, shape = "circle", hidden } = series[seriesId];
		if (hidden) return null;
		const axisIndex = disableHighlight ? void 0 : getHighlightedIndex.get(xAxisId);
		const showMarkIndex = showMark === "start" || showMark === "end" ? getStartEndMarkIndex(data, showMark) : void 0;
		const highlightedIndex = axisIndex ?? showMarkIndex;
		if (highlightedIndex === void 0 || data[highlightedIndex] == null) return null;
...
//#endregion
export { LineHighlightElement as a, LinePlot as c, AreaPlot as d, AreaElement as f, LineHighlightPlot as i, LineElement as l, AnimatedArea as m, FocusedLineMark as n, MarkPlot as o, lineClasses as p, LINE_CHART_PLUGINS as r, MarkElement as s, LineChart as t, AnimatedLine as u };
