...
const useGridDataSourceLazyLoader = (privateApiRef, props) => {
  const isNestedLazyLoadingEnabled = (0, _xDataGrid.useGridSelector)(privateApiRef, () => props.treeData ? true : (privateApiRef.current.unstable_applyPipeProcessors('getRowsParams', {})?.groupFields?.length ?? 0) > 0);
  const setStrategyAvailability = React.useCallback(() => {
    privateApiRef.current.setStrategyAvailability(_internals.GridStrategyGroup.DataSource, _internals.DataSourceRowsUpdateStrategy.LazyLoading, props.dataSource && props.lazyLoading && !isNestedLazyLoadingEnabled ? () => true : () => false);
  ...
  const handleStrategyActivityChange = React.useCallback(() => {
    setStrategyActive(privateApiRef.current.getActiveStrategy(_internals.GridStrategyGroup.DataSource) === _internals.DataSourceRowsUpdateStrategy.LazyLoading);
  }, [privateApiRef]);
  ...
  (0, _internals.useGridRegisterStrategyProcessor)(privateApiRef, _internals.DataSourceRowsUpdateStrategy.LazyLoading, 'dataSourceRootRowsUpdate', handleDataUpdate);
  ...
  (0, _xDataGrid.useGridEvent)(privateApiRef, 'rowsScrollEndIntersection', (0, _internals.runIf)(isStrategyActive, handleIntersection));
